...
(function (global, factory) {
                ...
                ...bindCellKeyAction$1(grid, cellId, {
                    action: (e) => {
                        ...
                        for (let row = selRange.start.row; row <= selRange.end.row; row++) {
                            ...
                            const cellRange = grid.getCellRange(col, row);
    ...
    const hasMusselScrollbar = () => { var _a; return !!((_a = window.mussel) === null || _a === void 0 ? void 0 : _a.scrollbar); };
    ...
    class InlineMenuElement {
        ...
        _bindMenuEvents() {
            const handler = this._handler;
            const menu = this._menu;
            const stopPropagationOnly = (e) => event.cancel(e); // 为了不在grid中传播
    ...
    function drawActionButton$1(grid, context, stateId, helper, { textBaseline, font, padding, appearance, color, }) {
        let result = false;
        const info = {
            hoverOnButton: false,
            hoverOnCell: false,
    ...
    class BaseColumn {
        ...
        onDrawCell(cellValue, info, context, grid) {
            const { style, getRecord, clearCellBase, drawCellBorder, drawCellBase, getCell, getContentHidden, } = info;
    ...
    class InlineMenuEditor extends Editor {
        constructor(options = {}) {
            ...
            this._autoPosition = isDef(options.autoPosition)
    ...
    class RadioEditor extends Editor {
        constructor(options = {}) {
            ...
            this._checkAction = options.checkAction;
        }
        clone() {
        ...
        _action(grid, cell) {
            ...
            grid.doGetCellValue(cell.col, cell.row, (value) => {
                ...
                if (this._checkAction) {
                    // User behavior
                    const record = grid.getRowRecord(cell.row);
                    this._checkAction(record, { cell, grid });
                    return;
                }
                if (this._group) {
    ...
    class SwitchColumn extends BaseCheckColumn {
        ...
        doDrawInternal(helper, value, context, record, opt) {
            ...
            const options = Object.assign(Object.assign({}, opt), { textOn,
                textOff });
    ...
    function _bindEvents() {
        ...
        handler.on(element, 'click', (e) => {
            if (this[_$2].columnResizer.lastMoving(e) ||
                this[_$2].selectionResizer.lastMoving(e) ||
        ...
        scrollable.onScroll((e) => {
            ...
            if (t instanceof Element) {
                const isScrollBottom = Math.abs(t.scrollHeight - t.scrollTop - t.clientHeight) <= 1;
                const isScrollRight = Math.abs(t.scrollWidth - t.scrollLeft - t.clientWidth) <= 1;
                this.fireListeners(DG_EVENT_TYPE.SCROLL, {
                    event: e,
                    scrollPosition: {
                        isTop: t.scrollTop === 0,
    ...
    class DrawGrid extends EventTarget {
        ...
        constructor(options = {}) {
            ...
            super();
            grids.push(this);
            const { rowCount = 10, colCount = 10, frozenColCount = 0, frozenRowCount = 0, defaultRowHeight, defaultColWidth, highlightBorderWidth, font, underlayRowCount = 0, underlayColCount = 0, underlayRowHeight = 'auto', underlayColWidth = 'auto', underlayBackgroundColor, borderMode = 'none', borderColor = '', borderWidth = 0, keyboardOptions, scrollbarOptions, parentElement, singleSelection, disableColumnResize, } = options;
    ...
    const scaleNum = (number = 0, position = 0) => {
    ...
    const DEFAULT_TOTAL_CAPTION = '小计';
    const PIVOT_DATA_SOURCE_EVENT_TYPE = Object.assign(Object.assign({}, DataSource.EVENT_TYPE), { REFRESH_HEADER: 'refresh_header' });
    const PIVOT_DATA_SOURCE_AGGREGATE_TYPE = {
        COUNT: 'count',
        SUM: 'sum',
        AVG: 'avg',
        MAX: 'max',
    ...
    const PIVOT_DATA_SOURCE_TOTAL_POSITION = {
        NONE: 'none',
        BEFORE: 'before',
        AFTER: 'after',
    };
    const PIVOT_DATA_SOURCE_DIM_SORT = {
        NONE: 'none',
        ASC: 'asc',
        DESC: 'desc',
    };
    function _getPivotRecordKey(record, dimensions, distinctKeys) {
        ...
        for (let dimIndex = 0; dimIndex < dimensions.length; dimIndex++) {
            const value = JSON.stringify(record[dimensions[dimIndex].field]) || '';
    ...
    function _genHeaderTotal(colDimensions, summaries, fieldMap, header, alwaysShowSummary, level = 0, prefixField = 'c', prefixKey = '') {
        var _a;
        const colDimension = colDimensions[level];
        const parentFieldInfo = (fieldMap[prefixKey] = fieldMap[prefixKey] || {});
        const totalOptions = {
        ...
        if (summaries.length <= 1 && !alwaysShowSummary) {
            const fieldName = prefixField + '_t';
            const columnDef = totalOptions;
            ...
            const columnDefs = [];
            _forEachSummaries(summaries, (summaryIndex, summary) => {
                const fieldName = prefixField + '_t_s_' + summaryIndex;
                columnDefs.push(_assignColDimColumnDefine({
                    field: fieldName,
                }, summary));
                parentFieldInfo[summaryIndex] = fieldName;
            });
            const groupHeaderDef = totalOptions;
            groupHeaderDef.columns = columnDefs;
            _assignColDimGroupDefine(groupHeaderDef, colDimension);
            header.push(groupHeaderDef);
        }
    }
    function _genHeader(colDimensions, colDistinctMap, summaries, fieldMap, alwaysShowSummary, level = 0, prefixField = 'c', prefixKey = '') {
        var _a;
        const header = [];
        if (colDimensions && colDimensions.length > 0) {
            ...
            const distinctValues = colDistinctMap[level];
            for (let i = 0; i < distinctValues.length; i++) {
                const distinctValue = distinctValues[i];
                const field = prefixField + '_' + i;
                const key = prefixKey ? prefixKey + '|' + distinctValue : distinctValue;
                let caption = JSON.parse(distinctValue);
                ...
                if (level === colDimensions.length - 1) {
                    const fieldInfo = (fieldMap[key] = fieldMap[key] || {});
                    if (summaries.length <= 1 && !alwaysShowSummary) {
                        fieldInfo[0] = field;
                        const columnDef = options;
                        ...
                        const columnDefs = [];
                        _forEachSummaries(summaries, (summaryIndex, summary) => {
                            const fieldName = field + '_s_' + summaryIndex;
                            columnDefs.push(_assignColDimColumnDefine({
                                field: fieldName,
                            }, summary));
                            fieldInfo[summaryIndex] = fieldName;
                        });
                        const groupHeaderDef = options;
                        groupHeaderDef.columns = columnDefs;
                        _assignColDimGroupDefine(groupHeaderDef, colDimension);
                    }
                }
            ...
            const rootFieldInfo = (fieldMap[''] = fieldMap[''] || {});
            _forEachSummaries(summaries, (summaryIndex, summary) => {
                const fieldName = 's_' + summaryIndex;
                header.push(_assignColDimColumnDefine({
                    field: fieldName,
                }, summary));
                rootFieldInfo[summaryIndex] = fieldName;
            });
        }
        return header;
    }
    ...
    function _forEachDim(dimensions, distinctMap, callback, totalCallback, level = 0, prefixKey = '', dimValues = {}) {
        if (dimensions && dimensions.length > 0) {
            const dim = dimensions[level];
            if (dim) {
                const distinctValues = distinctMap[level];
                if (totalCallback &&
    ...
    function _cellRangeMergeRow(rangeMap, startCol, startRow, records, rowDimCount, sameCount, endIndex, level) {
        // 合并行维单元格
        if (sameCount > 1) {
            const col = startCol + level;
            const startIndex = endIndex + 1 - sameCount;
            const range = {
                start: { col, row: startRow + startIndex },
                end: { col, row: startRow + endIndex },
            };
            for (let i = startIndex; i <= endIndex; i++) {
                rangeMap[`${col}|${startRow + i}`] = range;
            }
            _getCellRangeMap(rangeMap, startCol, startRow, records, rowDimCount, startIndex, endIndex, level + 1);
        }
    }
    function _getCellRangeMap(rangeMap, startCol, startRow, records, rowDimCount, fromIndex, toIndex, level = 0) {
        // 合并行维单元格
        if (level < rowDimCount) {
            ...
            for (let index = fromIndex; index <= toIndex; index++) {
                ...
                const totalLevel = Number(record['_total_level']);
                const isTotal = totalLevel >= 0 && totalLevel <= level;
                if (isTotal) {
                    ...
                    if (rowDimCount - 1 - totalLevel > 0) {
                        ...
                        for (let i = totalLevel; i <= rowDimCount - 1; i++) {
                            rangeMap[`${startCol + i}|${startRow + index}`] = range;
                        }
                    }
                    value = undefined;
                    sameCount = 0;
    ...
    function _sortDistinctKeys(distinctKeysMap, dimensions) {
        const distinctMap = {};
        for (const index in distinctKeysMap) {
            const sort = dimensions[index].sort;
            let keys = Object.keys(distinctKeysMap[index]);
            if (sort === 'asc') {
                keys = _encodeKeys(_decodeKeys(keys).sort((a, b) => {
    ...
    class PivotDataSource extends DataSource {
        ...
        get alwaysShowSummary() {
            return !!this._options.alwaysShowSummary;
        }
        getCellRangeMap(startCol, startRow) {
            const range = {};
        ...
        refreshInternal() {
            ...
            const keyRecordsMap = {};
            const rowDistinctKeys = {};
            const colDistinctKeys = {};
            const rowDimensions = this.rowDimensions || [];
            const colDimensions = this.colDimensions || [];
            const summaries = this.summaries || [];
            // 聚合记录
            for (let i = 0; i < this._dataSource.length; i++) {
                const record = this._dataSource.get(i);
                const rowKey = _getPivotRecordKey(record, rowDimensions, rowDistinctKeys);
                const colKey = _getPivotRecordKey(record, colDimensions, colDistinctKeys);
            ...
            // 获取维度值
            const rowDistinctMap = _sortDistinctKeys(rowDistinctKeys, rowDimensions);
            const colDistinctMap = _sortDistinctKeys(colDistinctKeys, colDimensions);
            // 聚合值
            const keyValueMap = {};
            for (const rowKey in keyRecordsMap) {
                const rowValueMap = (keyValueMap[rowKey] = keyValueMap[rowKey] || {});
                for (const colKey in keyRecordsMap[rowKey]) {
                    const colValueMap = (rowValueMap[colKey] = rowValueMap[colKey] || {});
                    const records = keyRecordsMap[rowKey][colKey];
                    _forEachSummaries(summaries, (summaryIndex, summary) => {
                        let value;
                        if (summary.aggregate === PIVOT_DATA_SOURCE_AGGREGATE_TYPE.COUNT) {
                            ...
                            if (summary.aggregate === PIVOT_DATA_SOURCE_AGGREGATE_TYPE.AVG) {
                                value = div(value, records.length);
                            }
                        }
            ...
            // 计算列头
            this._header = [];
            for (let rowDimIndex = 0; rowDimIndex < rowDimensions.length; rowDimIndex++) {
                const rowDim = rowDimensions[rowDimIndex];
                this._header.push(_assignRowDimColumnDefine({
                    field: ('r_' + rowDimIndex),
                }, rowDim));
            }
            const fieldMap = {};
            this._header.push(..._genHeader(colDimensions, colDistinctMap, summaries, fieldMap, this.alwaysShowSummary));
            // 初始化空字段标记
            const emptyFieldMap = {};
            _forEachDim(colDimensions, colDistinctMap, (colKey, colDimValues) => {
                _forEachSummaries(summaries, (summaryIndex) => {
                    const field = fieldMap[colKey][summaryIndex];
                    emptyFieldMap[field] = true;
                    _forEachParentKey(colDimensions, colDimValues, (parentKey) => {
                        if (fieldMap[parentKey]) {
                            const totalField = fieldMap[parentKey][summaryIndex];
            ...
            // 计算列维记录
            const recordMap = {};
            _forEachDim(rowDimensions, rowDistinctMap, (rowKey, rowDimValues) => {
                const record = {};
                let isEmptyRow = true;
                _forEachDim(colDimensions, colDistinctMap, (colKey, colDimValues) => {
                    _forEachSummaries(summaries, (summaryIndex) => {
                        const field = fieldMap[colKey][summaryIndex];
                        const value = keyValueMap[rowKey] &&
                ...
                if (!isEmptyRow || this.showEmptyRow) {
                    recordMap[rowKey] = record;
                    _forEachParentKey(rowDimensions, rowDimValues, (parentKey) => {
                        const totalRecord = (recordMap[parentKey] =
                            recordMap[parentKey] || {});
            ...
            // 计算行维记录
            const records = [];
            _forEachDim(rowDimensions, rowDistinctMap, (rowKey, dimValues) => {
                const record = recordMap[rowKey];
                if (record) {
                    for (let rowDimIndex = 0; rowDimIndex < rowDimensions.length; rowDimIndex++) {
            ...
            }, (totalLevel, totalKey, totalValues) => {
                var _a;
                const record = recordMap[totalKey];
                if (record) {
                    for (let rowDimIndex = 0; rowDimIndex < rowDimensions.length; rowDimIndex++) {
    ...
    const HEADER_STATE_ID$1 = getHeaderStateId();
    class HeaderAction extends BaseAction {
        ...
        bindGridEvent(grid, cellId) {
            const state = this.getState(grid);
            const action = (cell) => {
                if (!this._action || isHeaderDisabledCell(this.disabled, grid, cell)) {
                    return;
                }
                if (state.mouseActive) {
                    if (this._action) {
    ...
    class SortHeaderAction extends BaseAction {
        constructor(options = {}) {
            var _a;
            super(options);
            this._sort = (_a = options.sort) !== null && _a !== void 0 ? _a : true;
        }
        get sort() {
            return this._sort;
        }
...
.kaka-grid-header-icon-tooltip::after{
  ...
  margin-left: -8px;
    ...
    var headerUtils = {
        ...
        getIconPadding(context, helper, style) {
            const font = style.font;
            const padding = style.padding;
            const basePadding = helper.toBoxPixelArray(padding || 0, context, font);
            const textPadding = basePadding.slice(0);
        ...
        drawActionButton(grid, context, stateId, helper, style) {
            return drawUtils.drawActionButton(grid, context, stateId, helper, style);
        },
    };
    ...
    const HEADER_STATE_ID = getHeaderStateId();
    ...
    class Header extends BaseHeader {
        ...
        drawInternal(value, context, style, helper, grid, { drawCellBase, getHeaderIcon, getHeaderIconTooltip }) {
            ...
            headerUtils.loadIcons(getHeaderIcon(), context, helper, (icons, ctx) => {
                ...
                const hasBtn = headerUtils.drawActionButton(grid, context, HEADER_STATE_ID, helper, style);
                if (!isCustomDraw) {
                    const newPadding = iconTooltip || hasBtn
    ...
    function drawSwitchButton(ctx, x, y, check, animElapsedTime, { uncheckBgColor = '#FFF', checkBgColor = '#008CD6', borderColor = '#BFBFBF', btnSize = { width: 44, height: 22 }, textOn = '是', textOff = '否', } = {}) {
        animElapsedTime =
            animElapsedTime > 1 ? 1 : animElapsedTime < 0 ? 0 : animElapsedTime;
    ...
    class SimpleHeaderLayoutMap {
        ...
        _setupHeaderSpan() {
            const processed = {};
            for (let row = this._headerCellIds.length - 1; row >= 0; row--) {
                ...
                for (let col = cols.length - 1; col >= 0; col--) {
                    ...
                    if (!processed[cellId]) {
                        ...
                        if (rowSpan > 1 || colSpan > 1) {
                            let cellRange = this.getCellRange(col, row);
                            ...
                            for (let i = cellRange.start.row; i <= cellRange.end.row; i++) {
                                for (let j = cellRange.start.col; j <= cellRange.end.col; j++) {
    ...
    function _getBodyCellRange(grid, col, row) {
        ...
        if (spanBodyOptions && row >= layoutMap.headerRowCount) {
            cellRange = { start: { col, row }, end: { col, row } };
    ...
    function _getCellRange(grid, col, row) {
        const { layoutMap, cellRangeMap } = grid[_];
        let cellRange = layoutMap.getCellRange(col, row);
        if (cellRangeMap) {
            const range = cellRangeMap[`${col}|${row}`];
            if (range) {
    ...
    function _updateRect(grid, col, row, context) {
        ...
        context.setRectFilter((rect, range) => {
            let { left, right, top, bottom } = rect;
            const { start: { col: startCol, row: startRow }, end: { col: endCol, row: endRow }, } = range; // _getCellRange(grid, col, row) // 优化单元格渲染，减少_getCellRange调用频率
    ...
    function _tryWithUpdateDataSource(grid, fn) {
        ...
        grid[_].dataSourceEventIds = [
            grid[_].handler.on(grid[_].dataSource, PivotDataSource.EVENT_TYPE.REFRESH_HEADER, () => {
