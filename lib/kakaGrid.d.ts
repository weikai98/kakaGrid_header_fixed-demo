...
declare interface ActionOptions<T> extends BaseActionOptions<T> {
...
declare type Appearance = 'menulist-button' | 'lookup-button' | 'clear-button' | 'date-button' | 'none' | ((active: boolean, info: {
    hoverOnButton: boolean;
    hoverOnCell: boolean;
}) => SimpleColumnIconOption) | SimpleColumnIconOption;
...
declare class BaseAction<T> {
    ...
    constructor(options?: BaseHeaderActionOptions<T>);
...
declare abstract class BaseAction_2<T> {
    ...
    constructor(options?: BaseActionOptions<T>);
...
declare abstract class BaseActionInput<T> extends BaseInputEditor<T> {
    ...
    private _action?;
    constructor(options?: BaseActionInputOptions<T>);
...
declare interface BaseActionInputOptions<T> extends EditorOptions<T> {
    disableInput?: boolean | ActionListener<T>;
    disableAction?: boolean | ActionListener<T>;
    action?: ActionListener<T>;
}

declare interface BaseActionOptions<T> {
...
declare abstract class BaseCheckColumn<T> extends BaseColumn<T, boolean> {
    private _draw?;
    constructor(options?: BaseCheckColumnOptions<T>);
    ...
    protected abstract doDrawInternal(helper: GridCanvasHelperAPI, value: boolean, context: CellContext, record: T, opt: InlineCheckOption): void;
}

declare interface BaseCheckColumnOptions<T> extends BaseColumnOptions<T> {
...
declare interface BaseColumnOptions<T> {
...
declare abstract class BaseHeader<T> {
    private _draw?;
    constructor(options?: {
        draw?: DrawHeaderCallback;
    });
    get StyleClass(): typeof BaseStyle;
    get draw(): DrawHeaderCallback | undefined;
...
declare interface BaseHeaderActionOptions<T> {
...
declare class BranchGraphColumn<T> extends BaseColumn<T, unknown> {
    private _start;
    private _cache;
    constructor(options?: BranchGraphColumnOptions<T>);
...
declare interface BranchGraphColumnOptions<T> extends BaseColumnOptions<T> {
    start?: 'top' | 'bottom';
    cache?: boolean;
}
...
declare class ButtonAction<T> extends Action<T> {
    constructor(options?: ButtonActionOptions<T>);
    getState(grid: any): any;
}

declare type ButtonActionOptions<T> = ActionOptions<T>;

declare class ButtonColumn<T> extends Column<T> {
    private _caption?;
    private _linkButton;
    constructor(options?: ButtonColumnOptions<T>);
...
declare interface ButtonColumnOptions<T> extends ColumnOptions<T> {
    caption?: string;
    linkButton?: boolean;
}
...
declare interface CellContext {
    ...
    setRectFilter(rectFilter: (base: RectProps, range: CellRange) => RectProps): void;
}
...
declare interface CellRangeMap {
    [key: string]: CellRange;
}
...
declare class CheckColumn<T> extends BaseCheckColumn<T> {
    constructor(options?: CheckColumnOptions<T>);
    get StyleClass(): typeof CheckStyle;
    clone(): CheckColumn<T>;
    protected getState(grid: GridInternal<T>): CheckColumnState;
    protected doDrawInternal(helper: GridCanvasHelperAPI, value: boolean, context: CellContext, record: T, opt: InlineCheckOption): void;
}

declare interface CheckColumnOptions<T> extends BaseCheckColumnOptions<T> {
}
...
declare interface ColumnOptions<T> extends BaseColumnOptions<T> {
    draw?: DrawColumnCallback;
    cellStyle?: CellStyle;
}
...
declare class DateColumn<T> extends Column<T> {
    ...
    private _format?;
    constructor(options?: DateColumnOptions<T>);
...
declare interface DateColumnOptions<T> extends ColumnOptions<T> {
...
declare type DrawColumnCallback = <T>(value: any, ctx: CanvasRenderingContext2D, info: {
    grid: ListGridAPI<T>;
    record: unknown;
    rect: RectProps;
    col: number;
...
declare interface DrawColumnOptions<T> extends BaseColumnOptions<T> {
    draw?: DrawColumnCallback;
}
...
declare abstract class DrawGrid extends EventTarget_2 {
    ...
    get scrollbarOptions(): DrawGridScrollbarOptions | null;
    /**
     * Single selection.
     */
    ...
    protected _clearColumnsWidth(): void;
...
declare interface DrawGridConstructorOptions {
    ...
    scrollbarOptions?: DrawGridScrollbarOptions;
    /**
...
declare interface DrawGridScrollbarOptions {
    scrollbarVisible?: 'enter' | 'scroll' | false;
}

declare type DrawHeaderCallback = <T>(value: any, ctx: CanvasRenderingContext2D, info: {
    grid: ListGridAPI<T>;
}) => boolean;
...
declare function drawSwitchButton(ctx: CanvasRenderingContext2D, x: number, y: number, check: boolean, animElapsedTime: number, { uncheckBgColor, checkBgColor, borderColor, btnSize, textOn, textOff, }?: DrawSwitchOption): void;
...
declare interface EditorOptions<T> extends BaseActionOptions<T> {
...
declare interface GridCanvasHelperAPI {
    ...
    switch(check: boolean, context: CellContext, option: InlineSwitchOption): void;
    ...
    buildSwitchBtnInline(check: boolean, context: CellContext, option: InlineSwitchOption): InlineAPI;
...
declare class Header<T> extends BaseHeader<T> {
    ...
    drawInternal(value: string, context: CellContext, style: Style_2, helper: GridCanvasHelperAPI, grid: ListGridAPI<T>, { drawCellBase, getHeaderIcon, getHeaderIconTooltip }: DrawCellInfo<T>): void;
    bindGridEvent(grid: ListGridAPI<T>, cellId: LayoutObjectId): EventListenerId[];
}

declare class HeaderAction<T> extends BaseAction<T> {
    private _action?;
    constructor(options?: HeaderActionOptions<T>);
    clone(): HeaderAction<T>;
    get action(): HeaderActionListener<T> | undefined;
    set action(action: HeaderActionListener<T> | undefined);
    bindGridEvent(grid: ListGridAPI<T>, cellId: LayoutObjectId): EventListenerId[];
    protected getState(grid: any): any;
}

declare type HeaderActionListener<T> = (data: {
    grid: ListGridAPI<T>;
    cell: CellAddress;
}) => void;
...
declare interface HeaderActionOptions<T> extends BaseHeaderActionOptions<T> {
    action?: HeaderActionListener<T>;
}
...
declare class IconColumn<T> extends Column<T> {
    ...
    private _name?;
    private _iconWidth?;
    constructor(options?: IconColumnOptions<T>);
...
declare interface IconColumnOptions<T> extends ColumnOptions<T> {
    tagName?: string;
    className?: string;
    content?: string;
    name?: string;
...
declare class InlineInputEditor<T> extends BaseActionInput<T> {
    ...
    constructor(options?: InlineInputEditorOptions<T>);
...
declare interface InlineInputEditorOptions<T> extends EditorOptions<T> {
...
declare class InlineLookupEditor<T, L> extends BaseActionInput<T> {
    ...
    constructor(options?: InlineLookupEditorOptions<T, L>);
    get classList(): string[] | undefined;
    set classList(classList: string[] | undefined);
...
declare interface InlineLookupEditorOptions<T, L> extends BaseActionInputOptions<T> {
    classList?: string | string[];
...
declare class InlineMenuEditor<T> extends Editor<T> {
    ...
    constructor(options?: InlineMenuEditorOptions<T>);
    dispose(): void;
    get classList(): string[] | undefined;
    set classList(classList: string[] | undefined);
    ...
    set autoPosition(autoPosition: boolean);
...
declare interface InlineMenuEditorOptions<T> extends EditorOptions<T> {
    classList?: string | string[];
...
declare interface InlineSwitchOption extends InlineCheckOption {
    textOn?: string;
    textOff?: string;
}
...
declare class InlineTextAreaEditor<T> extends BaseActionInput<T> {
    private _classList;
    constructor(options?: InlineTextAreaEditorOptions<T>);
...
declare interface InlineTextAreaEditorOptions<T> extends BaseActionInputOptions<T> {
    classList?: string | string[];
}
...
declare interface IPivotBaseDimension {
    field: string;
    sort?: 'none' | 'asc' | 'desc' | ((a: any, b: any) => number);
    totalPosition?: 'none' | 'before' | 'after';
    totalCaption?: string;
    dimValueMap?: {
...
declare interface IPivotColDimension extends IPivotBaseDimension {
}

declare interface IPivotDataSourceOptions {
    rowDimensions?: IPivotRowDimension[];
    colDimensions?: IPivotColDimension[];
    summaries?: IPivotSummary[];
    showEmptyRow?: boolean;
    showEmptyCol?: boolean;
    alwaysShowSummary?: boolean;
}

declare interface IPivotRowDimension extends IPivotBaseDimension {
    width?: number | string;
    minWidth?: number | string;
    maxWidth?: number | string;
    disableResize?: boolean;
...
declare interface IPivotSummary {
    field: string;
...
declare class LookupColumn<T, L> extends BaseColumn<T, string> {
    ...
    private _draw?;
    private _cellStyle?;
    constructor(options?: LookupColumnOptions<T, L>);
...
declare interface LookupColumnOptions<T, L> extends BaseColumnOptions<T> {
    draw?: DrawColumnCallback;
    records?: L[] | ((rec: T) => L[] | Promise<L[]>);
    valueField?: string;
    captionField?: string;
...
declare class MenuColumn<T> extends BaseColumn<T, unknown> {
    private _options;
    constructor(options?: MenuColumnOptions<T>);
...
declare interface MenuColumnOptions<T> extends BaseColumnOptions<T> {
...
declare class MultilineTextColumn<T> extends BaseColumn<T, string> {
    private _draw?;
    private _cellStyle?;
    constructor(options?: MultilineTextColumnOptions<T>);
...
declare interface MultilineTextColumnOptions<T> extends BaseColumnOptions<T> {
    draw?: DrawColumnCallback;
    cellStyle?: CellStyle;
}
...
declare class MultilineTextHeader<T> extends BaseHeader<T> {
    ...
    drawInternal(value: string, context: CellContext, style: MultilineTextHeaderStyle, helper: GridCanvasHelperAPI, grid: GridInternal<T>, { drawCellBase }: DrawCellInfo<T>): void;
}
...
declare class NumberColumn<T> extends Column<T> {
    ...
    private _format?;
    constructor(options?: NumberColumnOptions<T>);
...
declare interface NumberColumnOptions<T> extends ColumnOptions<T> {
...
declare function of_5(headerStyle: HeaderStyleOption | null | undefined, headerValues: HeaderValues, StyleClass: typeof BaseStyle): BaseStyle;
...
declare class PercentCompleteBarColumn<T> extends Column<T> {
    ...
    private _max;
    private _formatter;
    constructor(options?: PercentCompleteBarColumnOptions<T>);
...
declare interface PercentCompleteBarColumnOptions<T> extends ColumnOptions<T> {
    min?: number;
    max?: number;
...
declare const PIVOT_DATA_SOURCE_AGGREGATE_TYPE: {
    COUNT: string;
    SUM: string;
    AVG: string;
    MAX: string;
...
declare const PIVOT_DATA_SOURCE_DIM_SORT: {
    NONE: string;
    ASC: string;
    DESC: string;
};

declare const PIVOT_DATA_SOURCE_EVENT_TYPE: {
...
declare const PIVOT_DATA_SOURCE_TOTAL_POSITION: {
    NONE: string;
    BEFORE: string;
    AFTER: string;
};
...
declare class PivotDataSource<T> extends DataSource<T> {
    static get EVENT_TYPE(): typeof PIVOT_DATA_SOURCE_EVENT_TYPE;
    static get AGGREGATE_TYPE(): typeof PIVOT_DATA_SOURCE_AGGREGATE_TYPE;
    static get TOTAL_POSITION(): typeof PIVOT_DATA_SOURCE_TOTAL_POSITION;
    static get DIM_SORT(): typeof PIVOT_DATA_SOURCE_DIM_SORT;
    private _dataSource;
    private _options;
    private _records;
    private _header;
    ...
    get dataSource(): DataSource<T>;
    get header(): HeadersDefine<T>;
    get rowDimensions(): IPivotRowDimension[] | undefined;
    get colDimensions(): IPivotColDimension[] | undefined;
    get summaries(): IPivotSummary[] | undefined;
    get showEmptyRow(): boolean;
    get showEmptyCol(): boolean;
    get alwaysShowSummary(): boolean;
    getCellRangeMap(startCol: number, startRow: number): CellRangeMap;
...
declare class RadioEditor<T> extends Editor<T> {
    ...
    constructor(options?: RadioEditorOptions<T>);
    clone(): RadioEditor<T>;
    ...
    get checkAction(): ActionListener<T> | undefined;
    set checkAction(checkAction: ActionListener<T> | undefined);
    bindGridEvent(grid: GridInternal<T>, cellId: LayoutObjectId): EventListenerId[];
    onPasteCellRangeBox(grid: GridInternal<T>, cell: CellAddress, value: string): void;
    onDeleteCellRangeBox(): void;
    private _action;
...
declare interface RadioEditorOptions<T> extends EditorOptions<T> {
    group?: GetRadioEditorGroup<T>;
    checkAction?: ActionListener<T>;
}
...
declare class SmallDialogInputEditor<T> extends BaseInputEditor<T> {
    ...
    private _classList?;
    private _type?;
    constructor(options?: SmallDialogInputEditorOptions<T>);
    dispose(): void;
    get classList(): string[] | undefined;
    set classList(classList: string[] | undefined);
    get type(): string | undefined;
...
declare interface SmallDialogInputEditorOptions<T> extends EditorOptions<T> {
    classList?: string | string[];
    type?: string;
...
declare class SortHeaderAction<T> extends BaseAction<T> {
    private _sort;
    constructor(options?: SortHeaderActionOptions<T>);
...
declare interface SortHeaderActionOptions<T> extends BaseHeaderActionOptions<T> {
    sort?: SortOption<T>;
}
...
declare class SwitchColumn<T> extends BaseCheckColumn<T> {
    private _textOn?;
    private _textOff?;
    constructor(options?: SwitchColumnOptions<T>);
    get textOn(): string | ((record: T) => string) | undefined;
    set textOn(value: string | ((record: T) => string) | undefined);
    get textOff(): string | ((record: T) => string) | undefined;
    set textOff(value: string | ((record: T) => string) | undefined);
...
declare interface SwitchColumnOptions<T> extends BaseCheckColumnOptions<T> {
    textOn?: string | ((record: T) => string);
    textOff?: string | ((record: T) => string);
}
...
declare class SwitchHeader<T> extends BaseCheckHeader<T> {
    ...
    get textOn(): string | undefined;
    set textOn(value: string | undefined);
    get textOff(): string | undefined;
    set textOff(value: string | undefined);
...
declare class TreeColumn<T> extends BaseColumn<T, string> {
    ...
    constructor(options?: TreeColumnOptions<T>);
...
declare interface TreeColumnOptions<T> extends BaseColumnOptions<T> {
    draw?: DrawColumnCallback;
    canToggle?: CanToggle;
    toggled?: Toggled;
    cellStyle?: CellStyle;
